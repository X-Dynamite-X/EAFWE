<x-layout.dashboard title="{{ __('modules.files.create') }}">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">{{ __('modules.files.create') }}</h1>
            <p class="text-gray-600 mt-1">{{ __('modules.files.fill_form') }}</p>
        </div>
        ...
        <x-ui.card>
            <form action="{{ route('dashboard.files.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-900 mb-2">{{ __('modules.files.fields.slug') }}
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label
                            ...
                            <span class="text-red-600">*</span></label>
                        <select
                            class="w-full px-4 py-2 border {{ $errors->has('file_type') ? 'border-red-500' : 'border-gray-300' }} rounded-lg focus:ring-2 focus:ring-blue-500"
                            id="file_type" name="file_type" required>
                            <option value="">{{ __('common.form.select_option') }}</option>
                            <option value="document" {{ old('file_type') == 'document' ? 'selected' : '' }}>
                                {{ __('modules.files.file_types.document') }}
                            </option>
                            <option value="pdf" {{ old('file_type') == 'pdf' ? 'selected' : '' }}>
                                {{ __('modules.files.file_types.pdf') }}</option>
                            <option value="guide" {{ old('file_type') == 'guide' ? 'selected' : '' }}>
                                {{ __('modules.marketing.resource_types.guide') }}</option>
                            <option value="template" {{ old('file_type') == 'template' ? 'selected' : '' }}>
