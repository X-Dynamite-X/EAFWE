<x-layout.dashboard title="{{ __('common.actions.add') }} ملف جديد">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">{{ __('common.actions.add') }} ملف جديد</h1>
            <p class="text-gray-600 mt-1">ملأ ال{{ __('modules.marketing.resource_types.template') }} أدناه ل{{ __('common.actions.create') }} ملف جديد</p>
        </div>
        ...
        <x-ui.card>
            <form action="{{ route('dashboard.files.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-900 mb-2">{{ __('common.general.file') }} <span
                                class="text-red-600">*</span></label>
                        <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-blue-500 transition"
                            ...
                            <input type="file" class="hidden" id="file" name="file"
                                accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.zip" required>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-900 mb-2">نوع {{ __('common.general.file') }} <span
                                class="text-red-600">*</span></label>
                        <select
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 @error('file_type') border-red-500 @enderror"
                            id="file_type" name="file_type" required>
                            <option value="">{{ __('common.form.select_option') }}</option>
                            <option value="document" {{ old('file_type') == 'document' ? 'selected' : '' }}>{{ __('modules.files.file_types.document') }}
                            </option>
                            <option value="pdf" {{ old('file_type') == 'pdf' ? 'selected' : '' }}>{{ __('modules.files.file_types.pdf') }}</option>
                            <option value="guide" {{ old('file_type') == 'guide' ? 'selected' : '' }}>{{ __('modules.marketing.resource_types.guide') }}</option>
                            <option value="template" {{ old('file_type') == 'template' ? 'selected' : '' }}>{{ __('modules.marketing.resource_types.template') }}
                            </option>
                        </select>
                        @error('file_type')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
