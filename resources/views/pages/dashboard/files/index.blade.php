...
<x-layout.dashboard title="{{ __('modules.files.title') }}">
    <div class="flex justify-between items-center mb-6">
        ...
        @can('manage member files')
            <x-ui.button href="{{ route('dashboard.files.manage') }}" color="primary">
                <i class="fas fa-cog"></i> {{ __('common.actions.manage') }} {{ __('modules.files.index') }}
            </x-ui.button>
        @endcan
    </div>
                ...
                <x-ui.card class="h-full flex flex-col hover:shadow-lg transition-shadow">
                    ...
                    <div class="mt-auto pt-4 border-t border-gray-200">
                        <a href="{{ route('dashboard.files.show', $file) }}"
                            class="inline-flex items-center text-green-600 hover:text-green-800 text-sm font-medium gap-1 w-full justify-center py-2 bg-green-50 rounded-lg hover:bg-green-100 transition">
    ...
    @else
        <x-ui.alert type="info" class="text-center">
            ...
            <p class="text-gray-700 font-medium">{{ __('modules.files.no_files') }}</p>
        </x-ui.alert>
    @endif
</x-layout.dashboard>
