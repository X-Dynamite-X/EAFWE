<x-layout.dashboard title="{{ __('modules.files.manage') }}">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">{{ __('modules.files.manage') }}</h1>
            <p class="text-gray-600 mt-1">{{ __('modules.files.subtitle') }}</p>
        </div>
        <x-ui.button href="{{ route('dashboard.files.create') }}" color="primary">
            <i class="fas fa-plus"></i> {{ __('modules.files.create') }}
        </x-ui.button>
    </div>
            ...
            @foreach ($files as $file)
                <x-ui.card class="h-full flex flex-col hover:shadow-lg transition-shadow">
                        ...
                        <div class="flex flex-wrap gap-2 mb-3">
                            <x-ui.badge color="green" size="sm">
                                @switch($file->file_type)
                                    @case('document')
                                        <i class="fas fa-file-word"></i> {{ __('modules.files.file_types.document') }}
                                    @break

                                    @case('pdf')
                                        <i class="fas fa-file-pdf"></i> {{ __('modules.files.file_types.pdf') }}
                                    @break

                                    @case('guide')
                                        <i class="fas fa-book"></i> {{ __('modules.files.file_types.guide') }}
                                    @break

                                    @case('template')
                                        <i class="fas fa-copy"></i> {{ __('modules.files.file_types.template') }}
                                    @break
                                @endswitch
                            </x-ui.badge>
                            @if ($file->file_size)
                    ...
                    <div class="mt-auto pt-4 border-t border-gray-200 flex gap-2">
                        ...
                        <a href="{{ route('dashboard.files.edit', $file) }}"
                        ...
                        <button type="button"
                            onclick="openDeleteModal('{{ $file->id }}', '{{ $file->title }}', '{{ route('dashboard.files.destroy', $file) }}')"
    ...
    @else
        <x-ui.card>
            <div class="text-center py-12">
                ...
                <p class="text-gray-600 text-lg font-medium">{{ __('modules.files.no_files') }}</p>
                <p class="text-gray-500 text-sm mt-2 mb-6">{{ __('modules.files.start_creating') }}</p>
                <x-ui.button href="{{ route('dashboard.files.create') }}" color="primary">
                    <i class="fas fa-plus"></i> {{ __('modules.files.create') }}
                </x-ui.button>
            </div>
