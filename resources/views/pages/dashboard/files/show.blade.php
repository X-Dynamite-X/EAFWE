<x-layout.dashboard title="{{ __('modules.files.show') }}">
    ...
    <div class="max-w-6xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <div>
                ...
                <p class="text-gray-600 mt-1">{{ __('modules.files.file_details') }}</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2">
                <x-ui.card>
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            @if ($file->file_size)
                                <span class="text-sm text-gray-600 flex items-center gap-1">
                                    ...
                                    {{ __('common.general.size') }}: {{ formatBytes($file->file_size) }}
                                </span>
                            @endif
                        </div>
                    </div>
                            ...
                            <div class="space-y-3">
                                @if (strpos($file->file_url, '/storage/') !== false)
                                    <div class="flex items-center gap-3 p-4 bg-blue-50 rounded-lg">
                                        ...
                                        <div>
                                            <p class="text-sm text-gray-600">{{ __('modules.files.uploaded_file') }}
                                            </p>
                                            <p class="text-sm font-medium text-gray-900">
                                                {{ basename($file->file_url) }}</p>
                                        </div>
                                    </div>
                                @endif
                                <a href="{{ $file->file_url }}" target="_blank"
                                    class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-6 rounded-lg transition">
                    ...
                    <div class="border-t border-gray-200 pt-4 text-sm text-gray-600">
                        ...
                        {{ __('common.time.created_at') }}: {{ $file->created_at->format('d/m/Y H:i') }}
            ...
            <div class="lg:col-span-1">
                <x-ui.card>
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('modules.files.file_info') }}</h3>
                    ...
                    <dl class="space-y-4">
                        ...
                        @if ($file->file_size)
                            <div class="border-t border-gray-200 pt-4">
                                <dt class="text-sm font-medium text-gray-600">{{ __('common.general.size') }}</dt>
                                <dd class="text-gray-900 mt-1">{{ formatBytes($file->file_size) }}</dd>
                            </div>
                        @endif

                        <div class="border-t border-gray-200 pt-4">
                            <dt class="text-sm font-medium text-gray-600">{{ __('common.general.status') }}</dt>
                            <dd class="text-gray-900 mt-1">
                        ...
                        <div class="border-t border-gray-200 pt-4">
                            <dt class="text-sm font-medium text-gray-600">{{ __('modules.files.fields.slug') }}</dt>
