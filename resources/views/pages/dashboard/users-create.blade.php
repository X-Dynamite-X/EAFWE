{{-- Create User Page --}}
<x-layout.dashboard title="{{ __('dashboard.users.create_title') }}">
    <div class="max-w-2xl mx-auto">
        <x-ui.card title="{{ __('dashboard.users.section_title') }}">
            <form action="{{ route('users.store') }}" method="POST">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <x-ui.input type="password" name="password" label="{{ __('dashboard.users.form.password') }}"
                        placeholder="••••••••" required />

                    <x-ui.input type="password" name="password_confirmation"
                        label="{{ __('dashboard.users.form.password_confirmation') }}" placeholder="••••••••"
                        required />
                </div>
                ...
                <div class="flex gap-4 mt-6">
                    <x-ui.button type="submit" color="gold" class="flex-1 text-center">
                        {{ __('dashboard.users.form.create_submit') }}
                    </x-ui.button>
