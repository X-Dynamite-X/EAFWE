<x-layout.dashboard title="{{ __('common.actions.add') }} {{ __('modules.portal.new_opportunity') }}">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-gradient-to-r from-cyan-50 to-blue-50 rounded-lg shadow-sm p-6 border border-cyan-100 mb-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-gray-900">{{ __('common.actions.add') }}
                        {{ __('modules.portal.new_opportunity') }}</h1>
                    <p class="text-gray-600 text-sm md:text-base mt-1">{{ __('modules.portal.start_creating') }}</p>
                </div>
            </div>
        </div>
                    ...
                    <div>
                        ...
                        <select
                            ...
                            @foreach (__('modules.portal.types') as $value => $label)
                                <option value="{{ $value }}"
                                    {{ old('opportunity_type') == $value ? 'selected' : '' }}>{{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select
                            ...
                            <option value="active" {{ old('status') == 'active' ? 'selected' : '' }}>
                                {{ __('common.status.active') }}</option>
