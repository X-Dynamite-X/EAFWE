<x-layout.dashboard title="تعديل الفرصة">
    <div class="max-w-2xl mx-auto">
        ...
        <x-ui.card>
            <form action="{{ route('dashboard.portal-opportunities.update', $opportunity) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                @method('PATCH')
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" name="opportunity_type" required>
                            <option value="business" {{ old('opportunity_type', $opportunity->opportunity_type) == 'business' ? 'selected' : '' }}>عمل تجاري</option>
                            <option value="partnership" {{ old('opportunity_type', $opportunity->opportunity_type) == 'partnership' ? 'selected' : '' }}>شراكة</option>
                            <option value="funding" {{ old('opportunity_type', $opportunity->opportunity_type) == 'funding' ? 'selected' : '' }}>تمويل</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" name="status" required>
                            <option value="active" {{ old('status', $opportunity->status) == 'active' ? 'selected' : '' }}>نشط</option>
                            <option value="closed" {{ old('status', $opportunity->status) == 'closed' ? 'selected' : '' }}>مغلق</option>
                            <option value="upcoming" {{ old('status', $opportunity->status) == 'upcoming' ? 'selected' : '' }}>قريباً</option>
                        </select>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="mb-3 p-3 bg-gray-100 rounded-lg">
                            <img src="{{ $opportunity->image_url }}" alt="{{ $opportunity->title }}" class="h-24 object-cover rounded">
                        </div>
                        @endif
                    ...
                    <div>
                        ...
                        <input type="number" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" name="order" value="{{ old('order', $opportunity->order) }}" min="0">
                    </div>
                </div>
