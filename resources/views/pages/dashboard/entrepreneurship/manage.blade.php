<x-layout.dashboard title="إدارة برامج الريادة">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">إدارة برامج الريادة</h1>
            <p class="text-gray-600 mt-1">أضف وعدل و{{ __('common.actions.delete') }} {{ __('modules.entrepreneurship.index') }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <x-ui.card class="h-full flex flex-col hover:shadow-lg transition-shadow">
            ...
            @else
            <div class="w-full h-40 bg-gradient-to-br from-yellow-100 to-yellow-50 rounded-t-lg flex items-center justify-center -m-4 mb-4">
                <i class="fas fa-rocket text-4xl text-yellow-300"></i>
            </div>
            @endif
                ...
                <div class="flex flex-wrap gap-2 mb-3">
                    <x-ui.badge color="yellow" size="sm">
                        @switch($program->type)
                            ...
                            @break
                            @case('startup')
                                <i class="fas fa-lightbulb"></i> {{ __('modules.entrepreneurship.types.startup') }}
                            @break
                            @case('mentorship')
                                <i class="fas fa-handshake"></i> إرشاد
                            @break
                        @endswitch
                    </x-ui.badge>
            ...
            <div class="mt-auto pt-4 border-t border-gray-200 flex gap-2">
                <a href="{{ route('dashboard.entrepreneurship.show', $program) }}" class="flex-1 text-center px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-lg transition text-sm">
                    <i class="fas fa-eye"></i> {{ __('common.actions.view') }}
                </a>
                <a href="{{ route('dashboard.entrepreneurship.edit', $program) }}" class="flex-1 text-center px-3 py-2 bg-blue-100 hover:bg-blue-200 text-blue-700 font-medium rounded-lg transition text-sm">
                    <i class="fas fa-edit"></i> {{ __('common.actions.edit') }}
                </a>
                <button type="button" onclick="openDeleteModal('{{ $program->id }}', '{{ $program->title }}', '{{ route('dashboard.entrepreneurship.destroy', $program) }}')" class="flex-1 text-center px-3 py-2 bg-red-100 hover:bg-red-200 text-red-700 font-medium rounded-lg transition text-sm">
    ...
    <x-ui.card>
        <div class="text-center py-12">
            ...
            <p class="text-gray-500 text-sm mt-2 mb-6">ابدأ ب{{ __('common.actions.create') }} برنامج ريادة جديد الآن</p>
            <x-ui.button href="{{ route('dashboard.entrepreneurship.create') }}" color="primary">
