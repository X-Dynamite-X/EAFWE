<x-layout.dashboard title="إدارة برامج الريادة">
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-gray-600 mt-1">{{ __('modules.entrepreneurship.add_edit_delete') }}</p>
        </div>
            ...
            @foreach ($programs as $program)
                <x-ui.card class="h-full flex flex-col hover:shadow-lg transition-shadow">
                    @if ($program->image_url)
                        <img src="{{ $program->image_url }}" class="w-full h-40 object-cover rounded-t-lg -m-4 mb-4"
                            alt="{{ $program->title }}">
                    @else
                        <div
                            class="w-full h-40 bg-gradient-to-br from-yellow-100 to-yellow-50 rounded-t-lg flex items-center justify-center -m-4 mb-4">
                    ...
                    <div class="flex-1 flex flex-col">
                        <div class="flex items-start gap-2 mb-2">
                            <span
                                class="inline-block px-3 py-1 bg-yellow-100 text-yellow-800 text-xs font-semibold rounded-full">
                                #{{ $program->id }}
                            </span>
                        ...
                        <div class="flex flex-wrap gap-2 mb-3">
                            <x-ui.badge color="yellow" size="sm">
                                @switch($program->type)
                                    ...
                                    @case('mentorship')
                                        <i class="fas fa-handshake"></i> {{ __('modules.entrepreneurship.types.mentorship') }}
                                    @break
                                @endswitch
                            </x-ui.badge>
                    ...
                    <div class="mt-auto pt-4 border-t border-gray-200 flex gap-2">
                        <a href="{{ route('dashboard.entrepreneurship.show', $program) }}"
                        ...
                        <a href="{{ route('dashboard.entrepreneurship.edit', $program) }}"
    ...
    @else
        <x-ui.card>
            <div class="text-center py-12">
                ...
                <p class="text-gray-600 text-lg font-medium">{{ __('modules.entrepreneurship.no_programs_create') }}
                </p>
