...
<x-layout.dashboard title="{{ __('common.actions.edit') }} المستخدم">
        ...
        <x-ui.card title="بيانات المستخدم">
            <form action="{{ route('users.update', $user) }}" method="POST">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <x-ui.input type="email" name="email" label="{{ __('common.general.email') }}" placeholder="user@example.com"
                        value="{{ old('email', $user->email) }}" required />
                </div>

                <x-ui.input name="phone" label="رقم {{ __('common.general.phone') }}" placeholder="+966 50 000 0000"
                ...
                <div class="mb-4">
                    <label class="flex items-center">
                        ...
                        <span class="mr-2 text-sm text-gray-700">{{ __('common.status.active') }}</span>
                    </label>
                </div>
