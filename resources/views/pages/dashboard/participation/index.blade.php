<x-layout.dashboard title="{{ __('modules.participation.title') }}">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">{{ __('modules.participation.index') }}</h1>
            <p class="text-gray-600 mt-1">{{ __('modules.participation.index_subtitle') }}</p>
        </div>
        @can('manage participation opportunities')
            <x-ui.button href="{{ route('dashboard.participation.manage') }}" color="primary">
                <i class="fas fa-cog"></i> {{ __('modules.participation.manage') }}
            </x-ui.button>
        @endcan
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($opportunities as $opportunity)
                <x-ui.card class="h-full flex flex-col hover:shadow-lg transition-shadow">
                    ...
                    <x-ui.button href="{{ route('dashboard.participation.show', $opportunity) }}" color="primary"
                        size="sm" class="mt-4 w-full text-center">
                        {{ __('modules.participation.opportunity_details') }} <i class="fas fa-arrow-left text-xs"></i>
                    </x-ui.button>
                </x-ui.card>
            @endforeach
        </div>
    @else
        <x-ui.card>
            <div class="text-center py-12">
                ...
                <p class="text-gray-600 text-lg">{{ __('modules.participation.no_opportunities') }}</p>
            </div>
        </x-ui.card>
    @endif
</x-layout.dashboard>
