<x-layout.dashboard title="{{ __('common.actions.view') }} {{ __('modules.marketing.title') }}">
    <div class="mb-6">
        <div class="flex justify-between items-center">
            <h1 class="text-3xl font-bold text-gray-900">{{ $resource->title }}</h1>
            @can('manage marketing resources')
                <div class="flex gap-2">
                    <a href="{{ route('dashboard.marketing.edit', $resource) }}"
                        class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition">
                        <i class="fas fa-edit"></i>
                        {{ __('common.actions.edit') }}
                    </a>
                    <button type="button"
                        onclick="openDeleteModal('{{ $resource->id }}', '{{ $resource->title }}', '{{ route('dashboard.marketing.destroy', $resource) }}')"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <x-ui.card class="mb-6">
                @if ($resource->image_url)
                    <img src="{{ $resource->image_url }}" class="w-full rounded-lg object-cover mb-6"
                        style="max-height: 400px;" alt="{{ $resource->title }}">
                @endif
                    ...
                    <x-ui.badge color="blue">
                        @switch($resource->resource_type)
                            @case('guide')
                                ...
                                ({{ __('modules.marketing.resource_types.guide_full') }})
                            @break

                            @case('template')
                                {{ __('modules.marketing.resource_types.template') }}
                                ({{ __('modules.marketing.resource_types.template_ready') }})
                            @break
                ...
                @if ($resource->file_url)
                    <div class="pt-6 border-t border-gray-200">
                        ...
                        <a href="{{ $resource->file_url }}" target="_blank"
                            class="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-lg transition">
                            <i class="fas fa-download"></i> {{ __('common.actions.download') }}
                            {{ __('common.general.file') }}
                        </a>
                    </div>
                @endif
            </x-ui.card>
            ...
            <div class="flex gap-2">
                <a href="{{ route('dashboard.marketing.index') }}"
                    class="flex-1 bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg text-center transition flex items-center justify-center gap-2">
        ...
        <div>
            <x-ui.card class="sticky top-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('modules.marketing.resource_info') }}</h3>

                <dl class="space-y-4">
                    <div>
                        <dt class="text-sm font-medium text-gray-700">
                            {{ __('modules.marketing.fields.resource_type') }}</dt>
                    ...
                    <div class="border-t border-gray-200 pt-4">
                        <dt class="text-sm font-medium text-gray-700">{{ __('common.general.type') }}</dt>
                        <dd class="mt-1">
                    ...
                    <div class="border-t border-gray-200 pt-4">
                        <dt class="text-sm font-medium text-gray-700">{{ __('modules.marketing.fields.slug') }}</dt>
                        <dd class="mt-1 text-sm font-mono text-gray-900 bg-gray-50 p-2 rounded">{{ $resource->slug }}
                        </dd>
                    </div>
                    ...
                    <div class="border-t border-gray-200 pt-4">
                        <dt class="text-sm font-medium text-gray-700">{{ __('common.time.created_at') }}</dt>
                        <dd class="mt-1 text-xs text-gray-500">{{ $resource->created_at->format('d/m/Y H:i') }}</dd>
                    </div>

                    @if ($resource->updated_at != $resource->created_at)
                        <div class="border-t border-gray-200 pt-4">
                            <dt class="text-sm font-medium text-gray-700">{{ __('common.time.updated_at') }}</dt>
                            <dd class="mt-1 text-xs text-gray-500">{{ $resource->updated_at->format('d/m/Y H:i') }}
                            </dd>
                        </div>
                    @endif
                </dl>
