<x-layout.dashboard title="{{ __('modules.marketing.title') }}">
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-gray-600 mt-1">{{ __('modules.marketing.subtitle') }}</p>
        </div>
        @can('manage marketing resources')
            <x-ui.button href="{{ route('dashboard.marketing.manage') }}" color="primary">
                <i class="fas fa-cog"></i> {{ __('modules.marketing.manage') }}
            </x-ui.button>
        @endcan
    </div>
            ...
            @foreach ($resources as $resource)
                <x-ui.card class="h-full flex flex-col hover:shadow-lg transition-shadow">
                    @if ($resource->image_url)
                        <img src="{{ $resource->image_url }}" class="w-full h-48 object-cover rounded-t-lg -m-4 mb-4"
                            alt="{{ $resource->title }}">
                    @else
                    ...
                    <div class="mt-auto pt-4 border-t border-gray-200">
                        <a href="{{ route('dashboard.marketing.show', $resource) }}"
                            ...
                            {{ __('common.actions.view') }} {{ __('common.general.file') }}
    ...
    @else
        <x-ui.alert type="info" class="text-center">
            ...
            <p class="text-gray-700 font-medium">{{ __('modules.marketing.no_resources') }}</p>
        </x-ui.alert>
    @endif
</x-layout.dashboard>
