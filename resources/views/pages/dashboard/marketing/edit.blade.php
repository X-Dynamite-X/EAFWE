<x-layout.dashboard title="{{ __('common.actions.edit') }} المورد التسويقي">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">{{ __('common.actions.edit') }} المورد التسويقي</h1>
        ...
        <x-ui.card>
            <form action="{{ route('dashboard.marketing.update', $resource) }}" method="POST"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-900 mb-2">نوع المورد <span
                                class="text-red-600">*</span></label>
                        <select
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                            name="resource_type" required>
                            <option value="guide"
                                {{ old('resource_type', $resource->resource_type) == 'guide' ? 'selected' : '' }}>{{ __('modules.marketing.resource_types.guide') }}
                                شامل</option>
                            <option value="template"
                                {{ old('resource_type', $resource->resource_type) == 'template' ? 'selected' : '' }}>
                                {{ __('modules.marketing.resource_types.template') }} جاهز</option>
                    ...
                    <div>
                        ...
                        @if ($resource->image_url)
                            <p class="text-xs text-blue-600 mb-2">الحالية: <img src="{{ $resource->image_url }}"
                                    alt="" class="w-16 h-16 rounded inline-block"></p>
                        @endif
                        <div class="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center cursor-pointer hover:border-blue-500 transition"
                            ...
                            <p class="text-sm text-gray-900">اسحب {{ __('common.general.image') }} أو اضغط</p>
                ...
                <div>
                    ...
                    @if ($resource->file_url)
                        <p class="text-xs text-blue-600 mb-2">{{ __('common.general.file') }} الحالي: <a href="{{ $resource->file_url }}"
                                target="_blank"
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-blue-500 transition"
                        ...
                        <p class="text-gray-900 font-medium">اسحب {{ __('common.general.file') }} هنا أو اضغط</p>
