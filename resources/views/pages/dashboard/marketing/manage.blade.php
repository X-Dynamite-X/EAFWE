<x-layout.dashboard title="{{ __('modules.marketing.manage') }}">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">{{ __('modules.marketing.manage') }}</h1>
            <p class="text-gray-600 mt-1">{{ __('modules.marketing.manage_subtitle') }}</p>
        </div>
        <x-ui.button href="{{ route('dashboard.marketing.create') }}" color="primary">
            <i class="fas fa-plus"></i> {{ __('modules.marketing.new_resource') }}
        </x-ui.button>
    </div>
            ...
            @foreach ($resources as $resource)
                <x-ui.card class="h-full flex flex-col hover:shadow-lg transition-shadow">
                    @if ($resource->image_url)
                        <img src="{{ $resource->image_url }}" class="w-full h-40 object-cover rounded-t-lg -m-4 mb-4"
                            alt="{{ $resource->title }}">
                    @else
                        <div
                            class="w-full h-40 bg-gradient-to-br from-green-100 to-green-50 rounded-t-lg flex items-center justify-center -m-4 mb-4">
                    ...
                    <div class="flex-1 flex flex-col">
                        <div class="flex items-start gap-2 mb-2">
                            <span
                                class="inline-block px-3 py-1 bg-green-100 text-green-800 text-xs font-semibold rounded-full">
                                #{{ $resource->id }}
                            </span>
                            ...
                            @else
                                <span
                                    class="inline-block px-3 py-1 bg-red-100 text-red-800 text-xs font-semibold rounded-full">
                        ...
                        <div class="flex flex-wrap gap-2 mb-3">
                            <x-ui.badge color="green" size="sm">
                                @switch($resource->resource_type)
                                    @case('guide')
                                        <i class="fas fa-book"></i> {{ __('modules.marketing.resource_types.guide') }}
                                    @break

                                    @case('template')
                                        <i class="fas fa-copy"></i> {{ __('modules.marketing.resource_types.template') }}
                                    @break
                    ...
                    <div class="mt-auto pt-4 border-t border-gray-200 flex gap-2">
                        <a href="{{ route('dashboard.marketing.show', $resource) }}"
    ...
    @else
        <x-ui.card>
            <div class="text-center py-12">
                ...
                <p class="text-gray-600 text-lg font-medium">{{ __('modules.marketing.no_resources') }}</p>
                <p class="text-gray-500 text-sm mt-2 mb-6">{{ __('modules.marketing.start_creating') }}</p>
                <x-ui.button href="{{ route('dashboard.marketing.create') }}" color="primary">
                    <i class="fas fa-plus"></i> {{ __('common.actions.add') }}
                    {{ __('modules.marketing.new_resource') }}
                </x-ui.button>
            </div>
        </x-ui.card>
    @endif
