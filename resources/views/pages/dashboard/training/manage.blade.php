<x-layout.dashboard title="{{ __('modules.training.manage') }}">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">{{ __('modules.training.manage') }}</h1>
            <p class="text-gray-600 mt-1">{{ __('modules.training.add_edit_delete') }}</p>
        </div>
        <x-ui.button href="{{ route('dashboard.training.create') }}" color="primary">
            <i class="fas fa-plus"></i> {{ __('modules.training.new_program') }}
        </x-ui.button>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($programs as $program)
                <x-ui.card class="h-full flex flex-col hover:shadow-lg transition-shadow">
                    ...
                    @else
                        <div
                            class="w-full h-40 bg-gradient-to-br from-blue-100 to-blue-50 rounded-t-lg flex items-center justify-center -m-4 mb-4">
                    ...
                    <div class="mt-auto pt-4 border-t border-gray-200 flex gap-2">
                        <a href="{{ route('dashboard.training.show', $program) }}"
                        ...
                        <a href="{{ route('dashboard.training.edit', $program) }}"
    ...
    @else
        <x-ui.card>
            <div class="text-center py-12">
                ...
                <p class="text-gray-600 text-lg font-medium">{{ __('modules.training.no_programs_create') }}</p>
                <p class="text-gray-500 text-sm mt-2 mb-6">{{ __('modules.training.start_creating') }}</p>
                <x-ui.button href="{{ route('dashboard.training.create') }}" color="primary">
                    <i class="fas fa-plus"></i> {{ __('modules.training.create') }}
                </x-ui.button>
            </div>
        </x-ui.card>
    @endif
