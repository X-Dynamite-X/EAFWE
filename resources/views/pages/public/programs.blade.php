<x-layout.app title="{{ __('البرامج والمبادرات') }}">

    {{-- Hero Section --}}
    <section class="bg-charcoal-900 text-white py-24 relative overflow-hidden">
        <div class="absolute inset-0 opacity-5">
            <span
                class="absolute -right-20 top-0 text-[18rem] font-black text-gold-500 select-none">
                {{ __('Programs') }}
            </span>
        </div>
    ...
    <section class="max-w-7xl mx-auto px-6 py-24">
        <x-ui.section-header
            :title="__('برامجنا التدريبية')"
            :description="__('برامج مصممة لدعم وتطوير المهارات الريادية')" />
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
            @foreach ($training_programs as $program)
                <div
                    ...
                    <div class="w-16 h-16 bg-gold-50 text-gold-500 text-3xl flex items-center justify-center rounded-2xl mb-6">
                        🎓
                    </div>
    ...
    <section class="max-w-7xl mx-auto px-6 py-24 bg-gold-50 rounded-[3rem]">
        <x-ui.section-header
            :title="__('برامج ريادة الأعمال')"
            :description="__('مبادرات تدعم إطلاق وتوسيع المشاريع الريادية')" />
