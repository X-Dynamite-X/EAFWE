    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8 flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600 mt-2">{{ __('member.card.card_preview') }}</p>
            </div>
            <div class="flex items-center gap-4">
                <x-language-theme-switcher />
                <a href="{{ route('member.profile') }}" class="text-blue-600 hover:text-blue-800">
                    ← {{ __('member.card.back') }}
                </a>
            </div>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-8">
            ...
            <div>
                <div
                    class="bg-gradient-to-br from-blue-500 via-purple-500 to-pink-500 rounded-2xl shadow-2xl p-8 text-white min-h-96 flex flex-col justify-between relative overflow-hidden">
                    {{-- Background Pattern --}}
                ...
                <x-ui.card class="mt-6">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between">
                            <span class="text-gray-600">{{ __('member.card.email') }}:</span>
                            <span class="font-semibold text-gray-900 ">{{ $user->email }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">{{ __('member.card.country') }}:</span>
                            <span class="font-semibold text-gray-900 ">{{ $membership->country }}</span>
                        </div>
                        @if ($membership->company_name)
                            <div class="flex justify-between">
                                <span class="text-gray-600">{{ __('member.card.company') }}:</span>
                                <span class="font-semibold text-gray-900 ">{{ $membership->company_name }}</span>
                            </div>
                        @endif
                        <div class="flex justify-between">
                            <span class="text-gray-600">{{ __('member.card.verification_status') }}:</span>
            ...
            <div>
                <x-ui.card>
                    ...
                    <div class="bg-gray-50 p-8 rounded-lg flex justify-center mb-6 border border-gray-200">
